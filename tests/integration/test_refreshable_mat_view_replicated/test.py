...
def module_setup_tables(started_cluster):
    ...
    node.query(
        "CREATE DATABASE IF NOT EXISTS default ENGINE=Replicated('/clickhouse/path/','{shard}','{replica}')"
    )
    ...
    node2.query(f"DROP DATABASE IF EXISTS test_db SYNC")
    node.query(f"CREATE DATABASE test_db")
